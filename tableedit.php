...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
			...
			$(document).ready(function(){
				$(".edit_tr").click(function(){
					...
					if(first.length && last.length>0){
						$.ajax({
							type: "POST",
							url: "inventory_table_edit_ajax.php",
							data: dataString,
							cache: false,
		...
		<link rel="stylesheet" href="assets/css/inventory_reset.css" type="text/css" media="screen" />
		<link rel="stylesheet" href="assets/css/inventory_tab.css" type="text/css" media="screen" />
		<link rel="stylesheet" type="text/css" href="assets/css/inventory_tcal.css" />
		<script type="text/javascript" src="assets/js/inventory_tcal.js"></script> 
		<link href="assets/css/inventory_tabs.css" rel="stylesheet" type="text/css" />
		<script type="text/javascript">
			var popupWindow=null;
			function child_open(){ 
				popupWindow =window.open('inventory_printform.php',"_blank","directories=no, status=no, menubar=no, scrollbars=yes, resizable=no,width=950, height=400,top=200,left=200");
			}
		</script>
	</head>
	<body bgcolor="#dedede">
		...
		<div class="content" id="inventory">
			...
			<table width="100%">
				...
				<?php } ?>
					<td class="edit_td"><span class="text"><?php echo $da; ?></span></td>
					<td><span class="text"><?php echo $item; ?></span></td>
					<td><span class="text"><?php echo $qtyleft; ?></span></td>
					<td>
						<span id="last_<?php echo $id; ?>" class="text">
						<?php
							$sqls=mysqli_query($link, "select * from sales where date='$da' and product_id='$id'");
							while($rows=mysqli_fetch_array($sqls)){
								echo $rows['qty'];
							}
						?>
						</span> 
						<input type="text" value="<?php if(isset($rtrt)):echo $rtrt;endif; ?>"  class="editbox" id="last_input_<?php echo $id; ?>"/>
					</td>
					<td>
					...
					<td>
						<span class="text">
						<?php
							if(isset($dailysales)):echo $dailysales;endif; 
							$sqls=mysqli_query($link, "select * from sales where date='$da' and product_id='$id'");
							while($rows=mysqli_fetch_array($sqls)){
								$rtyrty=$rows['qty'];
								$rrrrr=$rtyrty*$price;
								echo number_format($rrrrr,2,".",",");
							}
						?>
						</span> 
					</td>
			...
			$result1 = mysqli_query($link, "SELECT sum(sales) FROM sales where date='$da'");
		...
		<div class="content" id="alert">
			...
			<?php
				$CRITICAL=10;
				$sql2=mysqli_query($link, "select * from inventory where qtyleft<='$CRITICAL'");
		...
		<div class="content" id="addproitem">
			<form action="inventory_updateproduct.php" method="post">
		...
		<script src="assets/js/inventory_activatables.js" type="text/javascript"></script>
		<script type="text/javascript">
